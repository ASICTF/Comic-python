...
def main(inputPath):
    ...
    title = mypath.name
    outputFile = '<html><head><meta charset="UTF-8"><meta content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" name="viewport" /><title>' + title + '</title>\n'
    # CSS
    outputFile = outputFile + '<style>table{width:99%;}table,td{border:1px solid #000000;table-layout:fixed;border-collapse:collapse;}table td:first-child{width:80%;}a{color:#000000;text-decoration: none;}table tr:first-child{background-color:#eee;}tr:hover{background-color:#eee;}.folder{font-weight:bold;}.folderNameHide{display:none;}</style>\n'
    ...
    # Searching
    outputFile = outputFile + '<script type="text/javascript" language="JavaScript">function onSearch(){getCheckBox = document.getElementById(\'onlyShowFolderCheckBox\').checked;searchContent = document.getElementById("mySearch").value;var searchContentSC = transverter({type:"simplified",str:searchContent}).trim();var searchContentTC = transverter({type:"traditional",str:searchContent}).trim();var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;var fileCount = 0;var folderCount = 0;var fileSizeCount = 0;for(var i=1;i<rowsLength;i++){var searchText = storeId.rows[i].cells[0].innerHTML;var searchFileSize = storeId.rows[i].cells[1].innerHTML;if(searchText.match(searchContentTC) || searchText.match(searchContentSC) ||  searchText.toUpperCase().match(searchContent.toUpperCase())){if (searchText.match(\'folderNameShow\')){folderCount = folderCount + 1;}else{fileCount =  fileCount + 1;}if (searchText.match(\'folderNameShow\') && getCheckBox){storeId.rows[i].style.display=\'\';}else if(searchText.match(\'folderNameShow\') && !getCheckBox){storeId.rows[i].style.display=\'\';}else if(!searchText.match(\'folderNameShow\') && getCheckBox){storeId.rows[i].style.display=\'none\'}else if(!searchText.match(\'folderNameShow\') && !getCheckBox){storeId.rows[i].style.display=\'\'}if (searchFileSize.substring(searchFileSize.length-2) == \'KB\'){fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-2)) / 1024;}else if (searchFileSize.substring(searchFileSize.length-2) == \'MB\'){fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-2));}else if (searchFileSize.substring(searchFileSize.length-2) == \'GB\'){fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-2)) * 1024;}else if (searchFileSize.substring(searchFileSize.length-1) == \'B\'){fileSizeCount = fileSizeCount + parseInt(searchFileSize.substring(0,searchFileSize.length-1)) / 1024 / 1024;}}else{storeId.rows[i].style.display=\'none\';}}if (fileSizeCount > (1024 * 1024)){fileSizeCount = ((fileSizeCount / 1024 / 1024).toFixed(2)).toString() + \'TB\'}if (fileSizeCount > 1024){fileSizeCount = ((fileSizeCount / 1024).toFixed(2)).toString() + \'GB\'}if (fileSizeCount < 1024){fileSizeCount = fileSizeCount.toFixed(2).toString() + \'MB\'}document.getElementById("fileNameID").innerHTML = "<a href=\\"javascript:frontpage()\\">文件名 (关键词：\" +searchContent + \"，相关结果：\" + folderCount + \"个文件夹，\" + fileCount + \"个文件，共\" + fileSizeCount + \")</a> \";}function doClickFolder(){if (document.getElementById("fileNameID").innerHTML){var searchValue = event.target.innerHTML;searchValue = searchValue.split("\\\\").slice(-1);document.getElementById("mySearch").value = searchValue;onSearch();}}function frontpage(){start();document.getElementById("mySearch").value = "";var storeId = document.getElementById(\'allFileTable\');var rowsLength = storeId.rows.length;for(var i=1;i<rowsLength;i++){storeId.rows[i].style.display=\'\';}}</script>\n'
    ...
    outputFile = outputFile + '，' + time.strftime("%Y%m%d", time.localtime()) + ')</a> ";document.getElementById(\"tips\").innerHTML = \"<input onclick=\\"onSearch()\\" id=\\"onlyShowFolderCheckBox\\" type=\\"checkbox\\" value=\\"\\"/>只显示文件夹\";document.getElementById(\"mySearch\").focus();}start();var address = window.location.href.split(\"?search=\")[1];if (address){document.getElementById(\"mySearch\").value = decodeURI(address);onSearch();}start();</script>'
